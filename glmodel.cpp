...
void GLModel::modelCut(){
    ...
    if (cutMode == 1){ // flat cut
        ...
        bisectModelByPlane(lmesh, rmesh, parentModel->_mesh, parentModel->cuttingPlane, parentModel->cutFillMode);
        ...
        if (parentModel->cuttingPoints.size() >= 3){
            ...
            Mesh* rightMesh = parentModel->rmesh;
            // do bisecting mesh
            leftMesh->faces.reserve(parentModel->_mesh->getFaces().size()*10);
            leftMesh->vertices.reserve(parentModel->_mesh->getFaces().size()*30);
            rightMesh->faces.reserve(parentModel->_mesh->getFaces().size()*10);
            rightMesh->vertices.reserve(parentModel->_mesh->getFaces().size()*30);*/

            cutAway(lmesh, rmesh, parentModel->_mesh, parentModel->cuttingPoints, parentModel->cutFillMode);
