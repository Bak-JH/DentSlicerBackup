...
Mesh* ShellOffset::shellOffset(Mesh* mesh, float factor){
 ...
 //   }
    ...
    std::vector<FaceConstItr> unconnectedOffsetMeshFaces;
	auto faceCend = offsetMesh->getFaces().cend();
	for(auto mf = offsetMesh->getFaces().cbegin(); mf != faceCend; ++mf)
	{
        auto edge = mf.edge();
        for (size_t i = 0; i < 3; ++i, edge.moveNext()) {
            if (edge.nonOwningFaces().size() == 0)
            {
                unconnectedOffsetMeshFaces.push_back(mf);
                break;
            }
    ...
    for (int i=0; i<unconnectedOffsetMeshFaces.size(); i++){
        ...
        auto uomf = unconnectedOffsetMeshFaces[i];
