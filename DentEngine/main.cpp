...
//    int layer_num = round(slices.overhang_points[5].z()/scfg->layer_height);
//    qDebug() << slices.overhang_points[5].z() << slices.overhang_points[5].z()/scfg->layer_height << layer_num;
...
//    Paths contourList = slices[debug_layer].outershell;
    ...
    for (int contour_idx=0; contour_idx < contourList.size() ; contour_idx++){
        ...
        for (IntPoint ip : contour){
            //qDebug() << ip.X << ip.Y;
            p.drawLine(prev_ip.X*10/SlicingConfiguration::resolution + 500,prev_ip.Y*10/SlicingConfiguration::resolution + 500, ip.X*10/SlicingConfiguration::resolution + 500,ip.Y*10/SlicingConfiguration::resolution + 500);
            //p.drawPoint(ip.X*10/SlicingConfiguration::resolution + 500,ip.Y*10/SlicingConfiguration::resolution + 500);
            prev_ip = ip;
        }
        p.drawLine(prev_ip.X*10/SlicingConfiguration::resolution + 500,prev_ip.Y*10/SlicingConfiguration::resolution + 500, start_ip.X*10/SlicingConfiguration::resolution + 500,start_ip.Y*10/SlicingConfiguration::resolution + 500);
    }
    ...
    p.drawPoint(20*10/SlicingConfiguration::resolution, 500);
    p.drawPoint((20+scfg->branching_threshold_radius)*10/SlicingConfiguration::resolution, 500);
    ...
    for (OverhangPoint cop : slices.overhang_points){
        p.drawPoint(cop.position.X*10/SlicingConfiguration::resolution + 500, cop.position.Y*10/SlicingConfiguration::resolution + 500);
